...
// Replies definitions
const int SuccessReply = 0;
const int FailReply = 1;
const int WatchAlarm = 2;
  ...
  if (Serial.available() > 0) {
    ...
    if (Serial.read() == '\n') {
      switch (command) {
        case SetPinMode:
          pinMode(pin, option);
          Serial.println(SuccessReply, DEC);
          break;
        case GetDigital:
